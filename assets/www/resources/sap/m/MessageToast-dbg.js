...
sap.m.MessageToast._validateSettings = function(mSettings) {
	...
	if (typeof mSettings.duration !== "number" || !isFinite(mSettings.duration) || !(Math.floor(mSettings.duration) === mSettings.duration) || mSettings.duration <= 0) {
		throw new Error('"duration" needs to be a finite positive nonzero integer on ' + sMethodName);
	}

	// width
	if (!core.CSSSize.isValid(mSettings.width)) {
		throw new Error(mSettings.width + ' is not of type ' + '"sap.ui.core.CSSSize" for property "width" on ' + sMethodName);
	}

	// my
	if (typeof sMy !== "string" || !(sMy in Dock)) {
		throw new Error(mSettings.my + ' is not of type ' + '"sap.ui.core.Popup.Dock" for property "my" on ' + sMethodName);
	}

	// at
	if (typeof sAt !== "string" || !(sAt in Dock)) {
		throw new Error(mSettings.at + ' is not of type ' + '"sap.ui.core.Popup.Dock" for property "at" on ' + sMethodName);
	}

	// offset
	if (typeof mSettings.offset !== "string") {
		throw new Error(mSettings.offset + ' is of type ' + typeof mSettings.offset + ', expected "string" for property "offset" on ' + sMethodName);
	}

	// collision
	if (!rValidCollisions.test(mSettings.collision)) {
		throw new Error('"collision" needs to be a single value “fit”, “flip”, or “none”, or a pair for horizontal and vertical e.g. "fit flip”, "fit none", "flipfit"' + " on " + sMethodName);
	}

	// onClose
	if (typeof mSettings.onClose !== "function" && mSettings.onClose !== null) {
		throw new Error('"onClose" should be a function or null on ' + sMethodName);
	}

	// autoClose
	if (typeof mSettings.autoClose !== "boolean") {
		throw new Error('"autoClose" should be a boolean on ' + sMethodName);
	}

	// animationTimingFunction
	if (!rValidTimingFn.test(mSettings.animationTimingFunction)) {
		throw new Error('"animationTimingFunction" should be a string, expected values: ' + "ease, linear, ease-in, ease-out, ease-in-out on " + sMethodName);
	}
	...
	if (typeof mSettings.animationDuration !== "number" || !isFinite(mSettings.animationDuration) || !(Math.floor(mSettings.animationDuration) === mSettings.animationDuration) || mSettings.animationDuration < 0) {
		throw new Error('"animationDuration" needs to be a finite positive integer on ' + sMethodName);
	}
};
		...
		for (; i < self._aPopups.length; i++) {
			...
			if (oPopup instanceof sap.ui.core.Popup) {
				...
				if (jQuery.device.is.phone || jQuery.device.is.ipad) {
					oPopup.getContent().style.display = "none";

					setTimeout(function() {
						oPopup.setPosition(mPos.my, mPos.at, mPos.of, mPos.offset);
						oPopup.getContent().style.display = "block";
					}, 0);
				} else {
					oPopup.setPosition(mPos.my, mPos.at, mPos.of, mPos.offset);
				}
 ...
 * @param {string|sap.ui.core.Control|Element|jQuery} [mOptions.of=window] Specifies the reference element to which the message toast should be aligned, by default it is aligned to the browser visual viewport.
