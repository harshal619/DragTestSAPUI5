...
sap.m.Bar.prototype._updatePosition = function() {
	...
	// IE has a rounding issue with JQuery.outerWidth
		...
		for(var i= 0; i < aLBChildren.length ; i++){
			...
			iTotalWidth += parseFloat(oLBChildrenStyle.width);
			iTotalWidth += parseFloat(oLBChildrenStyle.marginLeft);
			iTotalWidth += parseFloat(oLBChildrenStyle.marginRight);
			iTotalWidth += parseFloat(oLBChildrenStyle.paddingLeft);
			iTotalWidth += parseFloat(oLBChildrenStyle.paddingRight);
		}
