...
//	recalculate the arrow position when the size of the popover changes.
	...
	// check if active element is within the popover, if not give popover the focus
	// reason: focus has to be inside/on popover otherwise autoclose() would not work
	var $activeElement = jQuery(document.activeElement);
	if (this.$().find($activeElement).length < 1) {
		var $this = jQuery(this.getDomRef());
		var $focusables = jQuery(":sapFocusable", $this);
		//set focus on popover if content has no focusable elements
		//set focus on first focusable element if content has focusable elements
